...
const PlaceItem = props =>{
    ...
    const confirmDeleteHandler = async ()=>{
        setShowConfirmModal(false)
        try {
            await sendRequest(`${process.env.REACT_APP_BACKEND_URL}/places/${props.id}`,'DELETE',{Authorization: `Bearer ${auth.token}`})
        ...
        <React.Fragment>
            ...
            >
                <div className="map-container">
                    <h1 className="center">MAP TO BE RENDERED USING GOOGLE MAP API</h1>
                </div>
            </Modal>
            <Modal
